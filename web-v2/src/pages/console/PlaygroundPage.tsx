...
function buildHeaders() {
  ...
  if (typeof userId === "number" && userId > 0) {
    headers["Aerspan-User"] = String(userId);
  }
  return headers;
}
...
function loadPlaygroundConfig(): {
  ...
  const raw = safeJsonParse<any>(
    localStorage.getItem(storageKeys.playgroundConfig),
  );
  return {
    ...
    customRequestBody:
      typeof raw?.customRequestBody === "string" ? raw.customRequestBody : "",
  };
}

function loadPlaygroundMessages(): PlaygroundMessage[] {
  const raw = safeJsonParse<any>(
    localStorage.getItem(storageKeys.playgroundMessages),
  );
      ...
      (m): PlaygroundMessage => ({
        ...
        reasoningContent:
          typeof m.reasoningContent === "string"
            ...
            : undefined,
        reasoningExpanded:
          typeof m.reasoningExpanded === "boolean"
            ? m.reasoningExpanded
            : undefined,
      }),
    );
...
function savePlaygroundMessages(messages: PlaygroundMessage[]) {
  localStorage.setItem(
    storageKeys.playgroundMessages,
...
export function PlaygroundPage() {
  ...
  const [parameterEnabled, setParameterEnabled] =
    useState<PlaygroundParameterEnabled>(initialConfig.parameterEnabled);
  const [systemPrompt, setSystemPrompt] = useState<string>(
    initialConfig.systemPrompt,
  );
  const [customRequestMode, setCustomRequestMode] = useState<boolean>(
    initialConfig.customRequestMode,
  );
  const [customRequestBody, setCustomRequestBody] = useState<string>(
    initialConfig.customRequestBody,
  );
  ...
  useEffect(() => {
    fetchJson<ApiResponse<UserModelItem[]>>("/api/user/models", {
  ...
  useEffect(() => {
    ...
    setInputs((prev) =>
      prev.model === preferredModelFromUrl
        ? prev
        : { ...prev, model: preferredModelFromUrl },
    );
  ...
  const updateInput = <K extends keyof PlaygroundInputs>(
    key: K,
    value: PlaygroundInputs[K],
  ) => {
  ...
  const stop = () => {
    ...
    setMessages((prev) => {
      ...
      if (last.status !== "loading" && last.status !== "incomplete")
        return prev;

      const parts = processIncompleteThinkTags(
    ...
    const appendAssistant = (delta: {
      ...
      setMessages((prev) => {
        ...
        const next: PlaygroundMessage = {
          ...
          status: "incomplete",
          reasoningContent:
            (last.reasoningContent || "") + (delta.reasoning || ""),
          ...
          <Card>
            ...
            <Card.Content className="space-y-3">
              <Select
                ...
                value={inputs.model}
                onChange={(value) =>
                  updateInput("model", String(value || inputs.model))
                }
              >
                <Label>Model</Label>
                <Select.Trigger>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput("temperature", Number(e.target.value))
                    }
                    className="w-28"
                  />
                </div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput("top_p", Number(e.target.value))
                    }
                    className="w-28"
                  />
                </div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput("max_tokens", Number(e.target.value))
                    }
                    className="w-28"
                  />
                </div>
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor="pg-param-frequency-penalty">
                      Frequency penalty
                    </Label>
                  </div>
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput("frequency_penalty", Number(e.target.value))
                    }
                    className="w-28"
                  />
                </div>
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor="pg-param-presence-penalty">
                      Presence penalty
                    </Label>
                  </div>
                  <Input
                    ...
                    onChange={(e) =>
                      updateInput("presence_penalty", Number(e.target.value))
                    }
                    className="w-28"
                  />
                </div>
                ...
                <Card.Content className="space-y-2">
                  <div className="flex items-center gap-3">
                    <Checkbox
                      ...
                      onChange={(isSelected) =>
                        updateInput("imageEnabled", isSelected)
                      }
                    >
                      <Checkbox.Control>
                        <Checkbox.Indicator />
              ...
              <Card variant="secondary">
                <Card.Content className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor="pg-custom-request-mode">
                      Custom request body
                    </Label>
                  </div>
                  {customRequestMode ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button
                          size="sm"
                          variant="secondary"
                          onPress={syncConversationToJson}
                        >
                          Sync → JSON
                        </Button>
                        <Button
                          size="sm"
                          variant="secondary"
                          onPress={applyJsonToConversation}
                        >
                          Apply JSON
                        </Button>
                      </div>
        ...
        <Card className="flex min-h-[60vh] flex-col gap-0 overflow-hidden p-0 lg:col-span-2">
          <Card.Content className="flex-1 space-y-3 overflow-auto p-4">
            ...
            {messages.map((m) => {
              ...
              const assistant =
                m.role === "assistant" ? getAssistantDisplayParts(m) : null;
              const variant =
                m.role === "user"
                  ? "secondary"
                  : m.role === "assistant"
          ...
          <div className="p-3">
            <div className="flex flex-col gap-2 md:flex-row">
              <TextArea
                ...
                placeholder={
                  customRequestMode
                    ? "Custom request mode is enabled (optional)."
                    : "Type a message…"
                }
                fullWidth
                  ...
                  onPress={() =>
                    send().catch((err) => {
                      ...
                      setMessages((prev) => {
                        ...
                        return [
                          ...prev.slice(0, -1),
                          { ...last, status: "error", content: String(err) },
                        ];
                      });
                      setSubmitting(false);
                      setIsStreaming(false);
