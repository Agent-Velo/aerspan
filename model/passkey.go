...
var (
	...
	ErrFriendlyPasskeyNotFound = errors.New("Passkey verification failed. Try again or contact an admin")
)
...
func UpsertPasskeyCredential(credential *PasskeyCredential) error {
	if credential == nil {
		...
		return fmt.Errorf("Couldn't save passkey. Try again")
	}
...
func DeletePasskeyByUserID(userID int) error {
	if userID == 0 {
		...
		return fmt.Errorf("Couldn't delete passkey. Try again")
	}
	// 使用Unscoped()进行硬删除，避免唯一索引冲突
