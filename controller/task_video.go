...
func updateVideoSingleTask(ctx context.Context, adaptor channel.TaskAdaptor, channel *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	// try parse as Aerspan response format
				...
				if modelName, ok := taskData["model"].(string); ok && modelName != "" {
					...
					if (hasModelPrice && modelPrice > 0) || (hasInputPrice && inputPrice > 0) {
						...
						if group != "" {
							...
							if quotaDelta > 0 {
								// 需要补扣费
								logger.LogInfo(ctx, fmt.Sprintf("Video task %s: charged extra %s after pre-charge (actual: %s, pre-charged: %s, tokens: %d)",
									task.TaskID,
								...
								if err := model.DecreaseUserQuota(task.UserId, quotaDelta); err != nil {
									logger.LogError(ctx, fmt.Sprintf("Failed to charge extra: %s", err.Error()))
								} else {
								...
								logger.LogInfo(ctx, fmt.Sprintf("Video task %s: refunded %s after pre-charge (actual: %s, pre-charged: %s, tokens: %d)",
									task.TaskID,
								...
								if err := model.RefundUserQuota(task.UserId, refundQuota); err != nil {
									logger.LogError(ctx, fmt.Sprintf("Failed to refund pre-charge: %s", err.Error()))
								} else {
									task.Quota = actualQuota // 更新任务记录的实际扣费额度
								...
								logger.LogInfo(ctx, fmt.Sprintf("Video task %s: pre-charge matched (amount: %s, tokens: %d)",
	...
	if shouldRefund {
		...
		if err := model.RefundUserQuota(task.UserId, quota); err != nil {
